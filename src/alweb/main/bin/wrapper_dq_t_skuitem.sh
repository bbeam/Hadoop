...
# Copy the dq_t_skuitem.properties file from S3 to HDFS and load the properties.
aws s3 cp s3://al-edh-dm/src/alweb/main/conf/dq_t_skuitem.properties /var/tmp/

if [ $? -eq 0 ]
then
	. /var/tmp/dq_t_skuitem.properties
else
	echo "Load of dq_t_skuitem.properties file failed from S3."
	exit 1
fi
...
pig \
	...
	-param_file /var/tmp/dq_t_skuitem.properties \
	-file s3://al-edh-dm/src/alweb/main/pig/dq_t_skuitem.pig \
	-useHCatalog
