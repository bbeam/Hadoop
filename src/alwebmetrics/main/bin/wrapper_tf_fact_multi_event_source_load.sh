...
################################################################################
# Name        : AngiesList                                                     #
# File        : wrapper_tf_fact_multi_event_source_load.sh                                             #
...
comma_cnt=`echo $TF_EVENT_KEY|grep -o ','|wc -l`

events_cnt=`expr $comma_cnt + 1`

for ((i=1;i<=events_cnt;i++)) do
DERIVED_TF_EVENT_KEY=`echo $TF_EVENT_KEY|cut -d "," -f$i`

# removal of existing data directories in work area
    if hadoop fs -test -d $TF_TABLE_WORK_LOCATION=$DERIVED_TF_EVENT_KEY; then
    
       hive -e "ALTER TABLE work_al_web_metrics.tf_nk_fact_web_metrics DROP PARTITION(event_type_key=$DERIVED_TF_EVENT_KEY)"
         if [ $? -eq 0 ]
         then
            echo "INFO:Dropping partition event_type_key=$DERIVED_TF_EVENT_KEY successful "
         else
            echo "ERROR:Dropping partition event_type_key=$DERIVED_TF_EVENT_KEY failed"
         exit 1
         fi
    
       echo "Removing transformation output data in work area for previous run.....Making $TF_DB.$TF_TABLE empty."
       hadoop fs -rmr $TF_TABLE_WORK_LOCATION=$DERIVED_TF_EVENT_KEY
         if [ $? -eq 0 ]
         then
            echo "INFO:Making $TF_TABLE_WORK_LOCATION=$DERIVED_TF_EVENT_KEY empty successful"
         else
            echo "ERROR:Making $TF_TABLE_WORK_LOCATION=$DERIVED_TF_EVENT_KEY empty failed."
            exit 1
         fi
    fi
done
