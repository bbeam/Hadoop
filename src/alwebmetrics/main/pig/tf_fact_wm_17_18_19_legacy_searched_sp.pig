/*#########################################################################################################
PIG SCRIPT                              :tf_fact_wm_17_18_19_legacy_searched_sp.pig
...
tf_searched_sp = FOREACH un_ss  GENERATE 
            ...
            (INT)1 AS (qty:INT),
			(search_params_type=='SP Search - category'?'17':(search_params_type=='SP Search - service_provider'?'18':(search_params_type=='SP Search - keyword'?'19':'$STRING_MISSING_VALUE'))) AS (event_type_key:CHARARRAY);
