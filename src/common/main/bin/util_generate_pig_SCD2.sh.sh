...
############################################################################################################################################################
echo "/*-----Filter out Current Active Records to compare and check old or new records------*/"$'\n'>>"$pig_file"
############################################################################################################################################################

echo "fltr_actv_master = FILTER load_master BY current_active_ind =='Y';"$'\n'>>"$pig_file";
...
then
        ...
        echo "join_source_master = JOIN md5_source BY ("$key_columns") FULL,fltr_actv_master BY ("$key_columns");"$'\n'>>"$pig_file";
        ...
        fltr_actv_master_key_columns=`echo "fltr_actv_master::$key_columns"|sed 's/,/,fltr_actv_master::/g'`
        ...
        echo "/*-----2.For Deleted records closed the eff_end_dt and as current_date the status of current active records and kepp the history records as it is   -------------*/"$'\n'>>"$pig_file";
        echo "/*-----3.For updates only process the history records as the current active records will closed using timestamp column present in the update        -------------*/"$'\n'>>"$pig_file";
        ...
        echo "join_cust_id = JOIN join_dist_partition by ($load_master_key_columns) LEFT , update_delete_record by ($fltr_actv_master_key_columns);"$'\n'>>"$pig_file";
...
############################################################################################################################################################
        ...
        echo "ToString(join_dist_partition::load_master::eff_start_dt, 'yyyy-MM-dd') AS eff_start_dt,">>"$pig_file"
        echo "ToString(join_dist_partition::load_master::eff_end_dt, 'yyyy-MM-dd') AS eff_end_dt,">>"$pig_file"
        echo "join_dist_partition::load_master::current_active_ind AS current_active_ind,">>"$pig_file"
...
############################################################################################################################################################
        ...
        echo "partiton_altered_update_records= FOREACH altered_update_record GENERATE">>"$pig_file";
        echo "join_dist_partition::load_master::s_key AS s_key,">>"$pig_file";
        ...
        echo "ToString(join_dist_partition::load_master::eff_start_dt, 'yyyy-MM-dd') AS eff_start_dt,">>"$pig_file";
        echo "(join_dist_partition::load_master::current_active_ind=='Y'?ToString(join_dist_partition::load_master::eff_start_dt, 'yyyy-MM-dd'):ToString(CurrentTime(), 'yyyy-MM-dd')) AS eff_end_dt,">>"$pig_file"
        echo "(join_dist_partition::load_master::current_active_ind=='Y'?'N':join_dist_partition::load_master::current_active_ind) AS current_active_ind,">>"$pig_file"
        echo "join_dist_partition::load_master::md5_non_key_value AS md5_non_key_value,">>"$pig_file"
        echo "join_dist_partition::load_master::md5_key_value AS md5_key_value,">>"$pig_file"
        ...
        echo "union_partition= UNION partiton_unaltered_records,partiton_altered_update_records,partiton_altered_delete_records;"$'\n'>>"$pig_file"
        ...
        echo "fltr_actv_master::s_key AS s_key,">>"$pig_file"
############################################################################################################################################################
## Generate Foreach column reference for key columns   ##
############################################################################################################################################################
...
############################################################################################################################################################
          ...
          if [[ "$timestamp_col" != "non" ]]
        then
         echo "ToString(fltr_actv_master::$timestamp_col, 'yyyy-MM-dd') AS eff_start_dt,">>"$pig_file";
		else  echo "ToString(CurrentTime(), 'yyyy-MM-dd') AS eff_start_dt,">>"$pig_file";
        fi
        
         echo "ToString(ToDate('9999-12-31'),'yyyy-MM-dd') as (eff_end_dt:chararray),">>"$pig_file";
         echo "'Y' as current_active_ind,">>"$pig_file";
...
############################################################################################################################################################
        ...
        echo "update_record_master = FOREACH filter_update_record GENERATE ">>"$pig_file";
        echo "fltr_actv_master::s_key AS s_key,">>"$pig_file";
...
############################################################################################################################################################
         ...
         while read line
         do
         echo "fltr_actv_master::$line AS $line,"
         done <"$key_file">>"$pig_file";
...
############################################################################################################################################################
        ...
        echo "ToString(fltr_actv_master::eff_start_dt, 'yyyy-MM-dd') AS eff_start_dt,">>"$pig_file";

############################################################################################################################################################
## Check if  timestamp_col is non,if not then include timestamp_col as eff_end_dt otherwise CurrentTime() ##
############################################################################################################################################################
		
        if [[ "$timestamp_col" != "non" ]]
        then
         echo "ToString(md5_source::$timestamp_col, 'yyyy-MM-dd') AS eff_end_dt,">>"$pig_file";
        else
         echo "ToString(CurrentTime(), 'yyyy-MM-dd') AS eff_end_dt,">>"$pig_file";
        fi
        
        echo "'N' AS current_active_ind,">>"$pig_file";
        echo "fltr_actv_master::md5_non_key_value AS md5_non_key_value,">>"$pig_file";
        echo "fltr_actv_master::md5_key_value AS md5_key_value,">>"$pig_file";
...
############################################################################################################################################################
        ...
        if [[ "$timestamp_col" != "non" ]]
        then
        echo "fltr_actv_master::$timestamp_col AS $timestamp_col,">>"$pig_file";
        fi
        echo "fltr_actv_master::$partition_col AS partition_col;"$'\n'>>"$pig_file";
		...
		if [[ "$timestamp_col" != "non" ]]
        then
         echo "ToString(insert_record_sk::md5_source::$timestamp_col, 'yyyy-MM-dd') AS eff_start_dt,">>"$pig_file";
        else
         echo "ToString(CurrentTime(), 'yyyy-MM-dd') AS eff_start_dt,">>"$pig_file";
        fi
...
############################################################################################################################################################
        ...
        echo "union_all= UNION insert_record_source,update_record_source,update_record_master,union_partition;"$'\n'>>"$pig_file";
        ...
        echo "ToDate(eff_start_dt) AS eff_start_dt,">>"$pig_file";
        echo "ToDate(eff_end_dt) AS eff_end_dt,">>"$pig_file";
        echo "current_active_ind AS current_active_ind,">>"$pig_file";
	    ...
	    ############################################################################################################################################################
         ...
         echo "join_source_master = JOIN md5_source BY ("$key_columns") LEFT,fltr_actv_master BY ("$key_columns");"$'\n'>>"$pig_file";
         
        
         echo "SPLIT join_source_master INTO "$'\n'" same_record IF (md5_source::md5_non_key_value==fltr_actv_master::md5_non_key_value), "$'\n'" update_record IF ((NOT(md5_source::md5_non_key_value==fltr_actv_master::md5_non_key_value)) AND fltr_actv_master::md5_key_value IS NOT NULL) ,"$'\n'" insert_record IF (fltr_actv_master::md5_key_value is null);"$'\n'>>"$pig_file";
        ############################################################################################################################################################
        echo "/***************************************Section 1 ends ****************************************************************/"$'\n'>>"$pig_file";		
	    ############################################################################################################################################################
	    ...
	    ############################################################################################################################################################
         ...
         echo "join_cust_id = JOIN join_dist_partition by ($load_master_key_columns) LEFT , update_record by ($fltr_actv_master_key_columns);"$'\n'>>"$pig_file";
         
         
         
         
         echo "SPLIT join_cust_id INTO"$'\n' "unaltered_record IF (update_record::fltr_actv_master::md5_key_value is null),"$'\n' "altered_record IF (NOT(update_record::fltr_actv_master::md5_key_value is null) AND join_dist_partition::load_master::current_active_ind != 'Y');"$'\n'>>"$pig_file";
	    ...
	    ############################################################################################################################################################
         ...
         echo "partiton_altered_records= FOREACH altered_record GENERATE">>"$pig_file";
         echo "join_dist_partition::load_master::s_key AS s_key,">>"$pig_file";
	    ...
	    ############################################################################################################################################################
         ...
         echo "union_partition= UNION partiton_unaltered_records,partiton_altered_records;"$'\n'>>"$pig_file";
	    ...
	    ############################################################################################################################################################
         ...
         echo "update_record_master = FOREACH update_record GENERATE ">>"$pig_file";
         echo "fltr_actv_master::s_key AS s_key,">>"$pig_file";
