...
Show_Usage()
{
        ...
        echo "Usage: "$0" <global properties file with path> <path of local properties file with path> <START_DATE(YYYY-MM-DD)> <END_DATE(YYYY-MM-DD)>"
        exit 1
}
if [ $# -ne 4 ]
then
...
sed -ie "s/EXTRACT_START_DATE/$EDH_BUS_DATE/g" /var/tmp/$OPTIONS_FILE_NAME
sed -ie "s/EXTRACT_END_DATE/$END_DATE/g" /var/tmp/$OPTIONS_FILE_NAME
