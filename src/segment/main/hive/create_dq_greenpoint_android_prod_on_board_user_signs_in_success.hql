...
--  DATE         : Jul 13, 2016
--  DESCRIPTION  : Creation of hive dq table(Dq_greenpoint_android_prod_on_board_user_signs_in_success). 
--  USAGE    : hive -f s3://al-edh-dev/src/segment/main/hive/create_dq_greenpoint_android_prod_on_board_user_signs_in_success.hql \
...
--  Creating a DQ hive table(Dq_greenpoint_android_prod_on_board_user_signs_in_success) over the incoming data
CREATE EXTERNAL TABLE IF NOT EXISTS ${hivevar:SEGMENT_GOLD_DB}.dq_greenpoint_android_prod_on_board_user_signs_in_success
( 
id  VARCHAR(254),
...
LOCATION '${hivevar:S3_BUCKET}/data/gold/${hivevar:SOURCE_SEGMENT}/greenpoint_android_prod/incremental/daily/dq_greenpoint_android_prod_on_board_user_signs_in_success';
