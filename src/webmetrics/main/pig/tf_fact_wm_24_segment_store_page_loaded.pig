/*
PIG SCRIPT    : tf_fact_wm_24_segement_store_page_loaded.pig
...
store_page_loaded_filtered = FILTER store_page_loaded BY edh_bus_date == '2016-01-01';
...
SPLIT store_page_loaded_filtered_mark_check_user_id INTO
                    store_page_loaded_user_id_missing IF (user_id == -1),
                    store_page_loaded_user_id_available IF (user_id != -1);
...
all_store_page_loaded_members_check_sp_id = FOREACH all_store_page_loaded_members GENERATE
                                id AS id,
                                est_sent_at AS est_sent_at,
                                request_info_id AS request_info_id,
                                qty AS qty,
...
SPLIT all_store_page_loaded_members_check_new_world_spid INTO
                    store_page_loaded_members_new_world_spid_missing IF (new_world_spid == -1),
                    store_page_loaded_members_new_world_spid_available IF (new_world_spid != -1); 
                                
/* Join with service_provider table to get the corresponding legacy_spid for a given new_world_spid */
jn_all_store_page_loaded_members_new_world_spid_available = FOREACH (JOIN review_members_new_world_spid_available BY new_world_spid LEFT , dim_service_provider BY new_world_spid ) 
